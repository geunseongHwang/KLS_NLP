...
def morpheme_analysis(x):  
    try:
        ...
        if x.strip() != '':
            ...
            # print(morpheme_anal)
            for v in morpheme_anal:
                if (v[0] not in stopword_list) and (len(v[0]) > 1):
                    # words += [v[0]]
                    words += [v]
    except:
        pass
...
def extract_word(x):
    ...
    try:
        ...
        words = []
        for k, v in x.items():
            if v == 'NNG' or v == 'NNP' or v == 'NNB' or v == 'VV' or v == 'VA':
                words.append(k)
    except SyntaxError:
        pass
    return words
...
def morpheme_extraction(x):
    ...
    dic_list = pd.DataFrame(senti_dic)['word'].tolist()
    snt_list = pd.DataFrame(senti_dic)['polarity'].tolist()
